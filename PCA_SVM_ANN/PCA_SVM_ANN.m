...
clc;
mtimes=10;
threshold = 1;% Threshold for selection of principal components
...
while testNO <mtimes
    ...
    for group=1:noGroup
        if trainModel==1 || trainModel==0
            % Linear SVM
            lnr_SVM{group} = svmtrain(double(label(train_idx)==group),dataPCA(train_idx,:), ...
                '-t 0 -b 1 -c 10 -h 0');
            % Make a p rediction for validation data
            [~, ~, p] = svmpredict(double(label(val_idx)==group),dataPCA(val_idx,:), lnr_SVM{group}, '-b 1');
            lnr_val_prob(:,group) = p(:,lnr_SVM{group}.Label==1);
            if trainModel==0
                % Make a p rediction for test data
                [~, ~, p] = svmpredict(double(label(test_idx)==group),dataPCA(test_idx,:), lnr_SVM{group}, '-b 1');
                lnr_prob(:,group) = p(:,lnr_SVM{group}.Label==1);
            end
        end
        if trainModel==2 || trainModel==0
            % Polynomial SVM
            poly_SVM{group} = svmtrain(double(label(train_idx)==group),dataPCA(train_idx,:), ...
                '-t 1 -b 1 -c 10 -d 1 -r 5 -h 0');
            % Make a p rediction for validation data
            [~, ~, p] = svmpredict(double(label(val_idx)==group),dataPCA(val_idx,:), poly_SVM{group}, '-b 1');
            poly_val_prob(:,group) = p(:,poly_SVM{group}.Label==1);
            if trainModel==0
                % Make a p rediction for test data
                [~, ~, p] = svmpredict(double(label(test_idx)==group),dataPCA(test_idx,:), poly_SVM{group}, '-b 1');
                poly_prob(:,group) = p(:,poly_SVM{group}.Label==1);
            end
        end
        if trainModel==3 || trainModel==0
            % RBF SVM
            rbf_SVM{group} = svmtrain(double(label(train_idx)==group),dataPCA(train_idx,:), ...
                '-t 2 -b 1 -d 1 -h 0');
            % Make a p rediction for validation data
            [~, ~, p] = svmpredict(double(label(val_idx)==group),dataPCA(val_idx,:), rbf_SVM{group}, '-b 1');
            rbf_val_prob(:,group) = p(:,rbf_SVM{group}.Label==1);
            if trainModel==0
                % Make a prediction for test data
                [~, ~, p] = svmpredict(double(label(test_idx)==group),dataPCA(test_idx,:), rbf_SVM{group}, '-b 1');
...
%         net.trainParam.time=10000;
        ...
        % Train the Network
        [net,tr] = train(net,dataPCA([train_idx val_idx],:)',labels([train_idx val_idx],:)');
        % Test the Network
